 ...
 * Genesis A2Z.
 *
 * This file adds the Genesis Connect for WooCommerce notice to the Genesis A2Z Theme.
 *
 * @package Genesis A2Z
 * @author  bobbingwide
...
add_action( 'admin_print_styles', 'genesis_a2z_remove_woocommerce_notice' );
/**
 * Removes the default WooCommerce Notice.
 *
 * @since 2.3.0
 */
function genesis_a2z_remove_woocommerce_notice() {
...
add_action( 'admin_notices', 'genesis_a2z_woocommerce_theme_notice' );
/**
 ...
 * @since 2.3.0
 */
function genesis_a2z_woocommerce_theme_notice() {
	...
	if ( get_user_option( 'genesis_a2z_woocommerce_message_dismissed', get_current_user_id() ) ) {
		return;
	}
...
add_action( 'wp_ajax_genesis_a2z_dismiss_woocommerce_notice', 'genesis_a2z_dismiss_woocommerce_notice' );
/**
 * Adds option to dismiss Genesis Connect for Woocommerce plugin install prompt.
 *
 * @since 2.3.0
 */
function genesis_a2z_dismiss_woocommerce_notice() {

	update_user_option( get_current_user_id(), 'genesis_a2z_woocommerce_message_dismissed', 1 );

}

add_action( 'admin_enqueue_scripts', 'genesis_a2z_notice_script' );
/**
 * Enqueues script to clear the Genesis Connect for WooCommerce plugin install prompt on dismissal.
 *
 * @since 2.3.0
 */
function genesis_a2z_notice_script() {

	wp_enqueue_script( 'genesis_a2z_notice_script', get_stylesheet_directory_uri() . '/lib/woocommerce/js/notice-update.js', array( 'jquery' ), '1.0', true );

}

add_action( 'switch_theme', 'genesis_a2z_reset_woocommerce_notice', 10, 2 );
/**
 * Clears the Genesis Connect for WooCommerce plugin install prompt on theme change.
 *
 * @since 2.3.0
 */
function genesis_a2z_reset_woocommerce_notice() {

	global $wpdb;

	$args  = array(
		'meta_key'   => $wpdb->prefix . 'genesis_a2z_woocommerce_message_dismissed',
		'meta_value' => 1,
	);
	$users = get_users( $args );

	foreach ( $users as $user ) {
		delete_user_option( $user->ID, 'genesis_a2z_woocommerce_message_dismissed' );
	}

}

add_action( 'deactivated_plugin', 'genesis_a2z_reset_woocommerce_notice_on_deactivation', 10, 2 );
/**
 ...
 */
function genesis_a2z_reset_woocommerce_notice_on_deactivation( $plugin, $network_deactivating ) {
	...
	genesis_a2z_reset_woocommerce_notice();
